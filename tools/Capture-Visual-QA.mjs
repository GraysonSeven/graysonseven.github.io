...
async function captureOne(route, theme, vp, group) {
  ...
  try {
    ...
    if (metrics.innerWidth !== vp.width || metrics.innerHeight !== vp.height) {
      throw new Error(`Viewport integrity mismatch: requested ${vp.width}x${vp.height}, got ${metrics.innerWidth}x${metrics.innerHeight}`);
    }

    const horizontalOverflow = Math.max(metrics.scrollWidth, metrics.bodyScrollWidth) - metrics.innerWidth;
    if (horizontalOverflow > 2) {
      layoutWarnings.push({
        group, route: route.name, theme, viewport: vp.name,
...
<div class="top"><h1>iCharles Visual QA — ${safeHtml(config.label)}</h1><p>${safeHtml(config.baseUrl)} · ${captures.length} captures · exact Playwright viewport emulation</p></div>
...
fs.writeFileSync(path.join(outRoot,"README.txt"),`ICHARLES AUTOMATED VISUAL QA
============================
